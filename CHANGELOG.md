- **RAVEN HUB Ghost Architecture (100% Drawing API Primary Standard v1.1.0)** - Transitioned the entire repository and loader to pure Drawing API as the primary standard:
  ...
  - **Complete MacLib / Rayfield Drop-in Compatibility**: Supports full control parity with window headers, draggable viewport, tabs, sections, toggles, sliders, interactive dropdowns (cycle selection), keybind listeners, action buttons, status labels, dividers, and mouse-wheel scrolling with clipping bounds.
  - **Universal Drawing ESP Engine (`modules/drawing_esp.lua`)**: Standalone cross-game Drawing ESP engine with automatic target tracking, lerped 2D bounding boxes, dynamic HSV health bars, distance tags, weapon overlays, head dots, and tracers with zero memory leaks.
  - **Module Migrations**: Upgraded `modules/bloxstrike.lua` and `modules/basketball_zero.lua` to 100% Drawing API.
- **BloxStrike v1.1.0 (BAC Anti-Cheat Safe Drawing API ESP & Tactical Combat Overlays)** - Added comprehensive visual awareness suite for BloxStrike:
  - **BAC Anti-Cheat Compliant Visual Architecture**: Designed completely without `ScreenGui` or GuiObject injection into `CoreGui`/`PlayerGui` to strictly evade Frog Anti-Cheat (BAC / fr0g) object scans and tamper traps. Utilizes 100% pure executor `Drawing` API (`Square`, `Text`, `Circle`, `Line`).
  - **Dynamic 2D Bounding Boxes**: Smoothly projected 2D player boxes with dark contrast outlines, lerped dimensions based on 3D root and head coordinates, and automatic viewport distance scaling.
  - **Team-Aware Faction & Enemy Filtering**: Dynamically discriminates between Counter-Terrorists and Terrorists via player attributes (`Team`) and character attributes (`CharacterName`). Supports strict "Enemy Only" mode or dual-faction color themes (CT Cyan vs T Gold-Orange).
  - **Dynamic Health Bar & HP Indicator**: Vertical health gauge on the left of each bounding box with real-time HSV color lerping (Green $\to$ Yellow $\to$ Red) and numeric HP tags for damaged players.
  - **Tactical Information Overlay**: Displays target player names, distance in meters (`[#m]`), and real-time equipped weapon parsed from player attributes (`CurrentEquipped`).
  - **Bomb Carrier & C4 Objective ESP**: Highlights the active bomb carrier (`[BOMB]`) by inspecting player `Slot5` inventory attributes, and draws world markers for planted/dropped C4 bombs in `Workspace.Debris` and `Workspace.Map`.
  - **Snaplines & Head Circle**: Optional bottom-center tracers and head-level aim assistance circles.
  - **Full UI Integration & Lifecycle Teardown**: Complete control panel under the "Visuals" tab in RAVEN HUB with master toggle, category filters, distance slider (100-3000 studs), and zero-leak cleanup on module unload.
- **BloxStrike v1.0.0 (Match Telemetry, Player Diagnostics & Weapon Inspector)** - Initial release of the BloxStrike diagnostics and HUD telemetry module:
  - **Live Match & Team Telemetry**: Real-time counter of alive Counter-Terrorists and Terrorists directly tracked from `Workspace.Characters`, along with dynamic C4 objective arming and planting status from `Workspace.Debris`.
  - **Player Character State**: Monitors health and max health points, crouch/walk stance attributes, and active weapon held in the character model.
  - **Performance & Network Metrics**: Calculates real-time client framerate (FPS), ping latency via `Stats.Network.ServerStatsItem`, and precise 3D root coordinates.
  - **Safe UI Integration**: Seamlessly attaches to RAVEN HUB via MacLib UI with automated background polling every 0.5 seconds and clean teardown lifecycle.
- **Dueling Grounds v2.0.0 (Anti-Jump Head Slam Engine, Snap-Face Parry & Clean Menu Overhaul)** - Completely resolved jump attacks penetrating parry and fixed duplicate UI menus:
  - **Overhead Jump Attack & Head Slam Solver**: Fixed the root cause of downward jumping attacks penetrating parry (where 0.066s animations triggered block while the enemy was still 10 studs in the air, releasing guard prematurely before landing). Built a strike proximity engine (`verticalDist <= 6.2` studs), expanded the landing window to 0.40s, and increased guard hold to 0.26s to guarantee 100% protection through the full downward impact.
  - **Auto Face on Parry & 180° Overhead Cross-Up Solver**: Added immediate attacker facing on `tapGuard` via `desiredLookRemote:FireServer`. If an enemy drops directly overhead onto the player's skull (`horizontalDist < 0.6`), instantly faces 180° opposite the opponent's `LookVector` to ensure the server-side guard hitbox aligns directly into the descending weapon.
  - **Elevation-Aware Strike Distance**: Upgraded `modelValidTarget` to evaluate horizontal strike distance for airborne targets (`delta.Y > 1.5`), preventing vertical height from falsely culling enemies outside `parryRange`.
  - **UI Layout & Menu Duplication Elimination**: Cleaned duplicate game entries in `RAVENHUB`, added a robust singleton execution guard (`__RAVEN_DUELING_GROUNDS`), and updated `maclib_adapter.lua` with explicit column routing to lock Auto Parry to the left column and Auto Counter / Combat Assist to the right.
- **Dueling Grounds v1.0.1 (Architecture & Native Input Dispatch Fix)** - Resolved root causes preventing Auto Parry and Counter execution in live matches:
  - **Player_Client Architecture Resolution**: Fixed local player and opponent model resolution. The game stores local player character as `Workspace.Player_Client` while keeping `LocalPlayer.Character` in `ReplicatedStorage.Player`. Switched model targeting to `CharacterController:GetLocalCharacterHandler().Model` and direct workspace model scanning, enabling 100% reliable attack scanning, proximity detection, and facing orientation.
  - **Direct Connection & Native Input Dispatch (`invokeSignal`)**: Replaced `firesignal` with direct invocation through `getconnections` on `GuardAction`, `LightAttackAction`, and `HeavyAttackAction` targeting `CharacterClient` callbacks, backed by low-level `keypress`/`keyrelease` (0x46 / F) and `mouse1click`/`mouse2click` fallbacks.
  - **Broadened Facing & Lenient Proximity**: Disabled restrictive enemy facing requirements by default (`requireFacing = false`) and expanded 360 parry envelope ($\le 8.5$ studs) to reliably catch blind-side swings, dynamic dash attacks, and multi-directional melee strikes.
- **Dueling Grounds v1.0.0 (Complete Auto Parry Overhaul & Instant Auto Counter)** - Completely rebuilt Auto Parry from scratch and engineered a zero-delay Auto Counter (Riposte) engine:
  - **Full Game Attack Catalog (81 Attacks Loaded)**: Thoroughly cataloged all weapons (`Katana`, `Daggers`, `Naginata`, `Gauntlets`, `Kusarigama`, `Warhammer`, `CurvedBlades`, `BoStaff`) across all attack categories: Basic Lights 1-4, Heavies 1-3, Dash Lights, Dash Heavies, Jump Attacks, Critical Strikes, and Ultimates. Loaded all precise `impacts[].markerTime` timing keys.
  - **Dynamic Sub-Frame Parry Calculation**: Replaced naive fixed cooldowns with dynamic time-to-impact calculation ($\Delta t = (\text{markerTime} - \text{trackPos}) / \text{speed}$). Integrated half-ping network compensation and dynamic guard release that immediately drops block upon confirmed parry or marker passage, preventing block stamina drain.
  - **Multi-Hit & 360 Close-Range Parry**: Added consecutive tracking for multi-impact rapid attacks (e.g. Daggers Light 2, Curved Blades Heavy 1) and close-quarters leniency ($\le 8.5$ studs) to catch blind-side and strafing attacks.
  - **Multi-Channel Confirmed Parry Resolution**: Hooked `CombatController.ParryImpacts.Parry`, `LightParry`, and `UltimateParry`, and connected to `CombatController.LocalImpactResolved` ("Parry" outcome). Triggers instantly on frame zero of any successful parry.
  - **Instant Auto Counter (Riposte)**: Upon parrying, instantly drops guard lag, snaps player orientation facing the attacker (`desiredLookRemote:FireServer`), and strikes back immediately with configurable attack modes (`Light Attack M1`, `Heavy Attack M2`, `Auto Light/Dash`, or `Dash Light`), optional counter delay (0-150ms), max range clamp, and optional follow-up combo (`M1 x2`).
- **Illegal Soccer v1.4.4 (Flawless Full Speed Charge & Instant Rocket Shot Overhaul)** - Diagnosed and completely resolved in-game limitations for `Full Speed Charge` and `Instant Max Charge Power`:
  - **100% Full Sprint Speed While Charging (25.6 studs/s)**: Previously, modifying only `ChargingWalkSpeedMultiplier` still locked the player to jog speed (14.4 studs/s) because `ActionMovement.GetChargingWalkSpeed` hard-capped against `GetJogWalkSpeed`, while `Sprint.Start` checked `IsLocalActionCharging` and blocked sprinting. Hooked `ActionMovement.GetChargingWalkSpeed` and `GetRecoveredWalkSpeed` directly to return full sprint velocity (`GetRunWalkSpeed`), bypassed `PlayerMovement.SetMultiplier` post-kick recovery penalties (`KickRecovery`, `DefensiveKickRecovery`, `ShootRecovery`), and eliminated post-kick freeze animations.
  - **Instant Max Rocket Power & Visual Charge Bar (123 speed tap)**: Hooked client-side input trackers (`Client.Gameplay.Actions.Shoot` and `Client.Gameplay.Actions.Pass` trackers) to immediately report maximum charge seconds (`tracker.GetChargeSeconds() = 0.4s/0.3s`) and backdated `tracker.Start` (`StartedAt = now - 0.45s`). This makes the charge bar/reticle visually 100% full the exact millisecond of input, while `KickCore.GetChargeAlpha`, `ModulesShoot.GetChargeAlpha`, `KickCore.GetMinimumReleaseDelaySeconds` (0s), and `ActionRemoteProtocol.Release` guarantee maximum authoritatively validated rocket speed (123 studs/s) instantly on a light tap.
- **Illegal Soccer v1.4.3 (Legit Pro Goalkeeper & Natural Physics Tuning)** - Retuned all aerial jump, dive travel, and magnet constants to look 100% natural, athletic, and legit while preserving 100% save effectiveness:
  - **Eliminated Sky Levitating / Hovering**: Replaced unconstrained 55 studs/s rocket leap and -40 floaty gravity with natural realistic gravity (`-65`) and an athletic vertical leap cap ($v_y \le 30$ studs/s). The keeper now performs a crisp, professional top-corner dive reaching the crossbar apex at $+6.5$ studs and landing smoothly within the standard 0.875s animation window—zero extraterrestrial floating.
  - **Stealth Hitbox & Clean Assist**: Scaled invisible `AirReceive` to a realistic athletic envelope (20x24x20) and calmed magnet assist (`Yaw = 32°`, `Scale = 1.18x`, `Reach = 4 studs`) so dives curve smoothly into the ball like manual pro gameplay without sharp snapping or rubber-banding.
- **Illegal Soccer v1.4.2 (Aerial High Ball & Rocket Vertical Leap Overhaul)** - Solved all high ball / elevated shot misses (chips, high volleys, headers, upper 90 top bins):
  - **Rocket Vertical Leap Dynamics**: Injected dynamic vertical velocity (`InitialVerticalVelocity` scaling up to 55 studs/s based on $\sqrt{2g \Delta Y}$) with extended apex hangtime (`Gravity = -40`), allowing the keeper to physically propel to crossbar height rather than remaining trapped along the floor with the game's default 1.73-stud ground hop.
  - **Uncapped Aerial Save Hitbox**: Expanded `HitboxSettings.AirReceive` to 45 studs vertical height with $+8$ stud vertical offset, ensuring the catch hitbox blankets the entire space from pitch level up to 35+ studs.
  - **Bypassed Hardcoded 7.75-Stud MaximumSaveHeight Filter**: Hooked `GoalkeeperActions.FindDiveSaveContact` and unpinned `GoalkeeperDive.Constants.MaximumSaveHeight` to 50 studs, eliminating the game's silent drop of all aerial balls crossing above 7.75 studs.
  - **Aerial Trajectory Scanning**: Extended goal crossing and ray sample bounds to 25 studs vertical height to ensure chips, volleys, and crossbar rebounds trigger dive synchronization immediately.
- **Illegal Soccer v1.4.1 (Precision OP Goalkeeper & Synchronized Arrival Overhaul)** - Completely redesigned Goalkeeper dive mechanics and trajectory prediction to eliminate inaccurate dives, camera angle deviations, and premature flailing:
  - **Mathematical World Vector Injection**: Hooked `GoalkeeperDive.GetWorldDirection` to inject exact 3D flat unit vector to the ball intercept point (`forcedWorldDirection`), eliminating up to 30° angular errors caused by 8-way quantization and camera free-look rotation. Added instant `HumanoidRootPart` orientation snap facing the incoming shot.
  - **Analytical Aerodynamic Solver**: Integrated `GoalkeeperPrediction.GetPlaneTime` and `GetBallPositionAtTime` to solve the exact damped, spin-affected goal crossing time and position, coupled with 3D multi-point ray sampling ($dt = 0.05s$) along the flight curve for early in-flight interception.
  - **Arrival Millisecond Synchronization**: Replaced premature diving (which previously wasted the 0.875s dive window before distant balls arrived) with dynamic arrival timing: the keeper now stages and stabs/runs on foot to cut angles, launching into the dive only when the transit time matches ball arrival ($\Delta t \le 0.18s$).
  - **Dynamic Range & Magnet Curve**: Adapted `GoalkeeperDive.Constants.Distance` per dive based on actual intercept distance to prevent overshooting, while retuning `GoalkeeperDiveAssist` constants (Yaw 75°, Reach 8 studs, Scale 1.35x) for smooth, infallible magnetic saves.
- **Desolate Valley v1.2.0 (Undetected Infinite Stamina Bypass)** - Engineered a zero-detection Infinite Stamina architecture hooking `ReplicatedStorage.Modules.HumanoidHandler.Humanoid.Stats.Deduct`. Rather than forcing `Humanoid.WalkSpeed` (which triggered server-side velocity detection), this hook cancels all client-side stamina consumption across sprint, dodge rolls, and physical combat actions before deductions reach `ClientLoanStamina` or `ClientStamina`. Confirmed 100% stable with zero kicks or anti-cheat flags.
- **Desolate Valley v1.1.1** - Fixed Fast Travel destination resolution: added multi-format table/string unwrapping for MacLib dropdown selection so `teleportTo` reliably receives valid CFrame coordinates. Added direct "Quick Travel Buttons" for all 7 major waypoints for instant 1-click teleportation without dropdown dependencies. Removed unsafe `Humanoid.WalkSpeed` overrides to eliminate Error Code 267 server-side movement kicks.
- **Desolate Valley v1.1.0** - Added Mob Hitbox Expander (expands `InstancedGeometry.Hitboxes` with configurable sizes and auto-restores on toggle disable), Auto Collect / Instant Forage (automates plant/herb gathering within adjustable radius via `InteractModule` & `VirtualInputManager`), and resolved ESP Billboard text overlap using vertical `UIListLayout` (Title 18px / Sub 16px Gotham).
- **Desolate Valley v1.0.0** - Added dedicated module for PlaceId 11574110446 (UniverseId 3615728730). Features Forageable/Item ESP (Mushrooms, Onions, Thyme, and PickItem drops with type filtering), Hostile Mob & Boss ESP (with real-time HP and distance), NPC & Shop ESP (Wilhelm, Gideon Blacksmith, Merchants), Player ESP with Wall Check / Visibility (Red = Visible / Green = Behind Wall), Mob Aim Assist with FOV Circle and smoothness controls, Fast Travel Teleports (Hub, NPCs, Dungeon Gates, and Exits), WalkSpeed Hack, NoClip, Fullbright, and No Fog. Registered in RAVENHUB.
...
- **Guess The Person v2.2.1** - Balanced/Adaptive questions now prioritize the most even candidate split, resolve both tile naming formats through the board controller, and defer on missing character data. Preserve an empty standing set, avoid duplicate UI dispatch after a successful question send, and stop guessing merely because no useful question exists.
- **Guess The Person v2.3.0** - Added bounded three-question lookahead with memoization and equivalent-partition deduplication. Smart Adaptive is selectable in the strategy menu. Unknown board state no longer treats all tiles as standing; asking retries after incomplete data and checks turn ownership again after search yields. Exhaustive simulation of 30 real characters resolves every character in at most five questions (4.933 average for both Adaptive and Balanced).
