...
- ปรับ `Auto Mine` ให้ดึงราคา/`RequiredStrength` จากอินสแตนซ์แร่ในเกม (แอตทริบิวต์, Value, ข้อความ UI เช่น `needs … strength`, ProximityPrompt) แทนตาราง `Ore list`; ยังคัดตามดาเมจ pickaxe, เรียงเป้าตามราคาที่อ่านได้แล้วระยะ, แคชสั้นๆ ต่อก้อน, และสถานะ `pickaxe too weak for nearby ores` เมื่อมีแร่ในระยะแต่ขุดไม่ได้ทั้งหมด
