...
- ปรับ `Auto Mine` ให้ใช้ราคาขายจากตารางอ้างอิงในโค้ด (`oreReferenceSellPrice`, sync กับ repo `Ore list`) สำหรับการเรียงเป้า ส่วน `RequiredStrength` ยังอ่านจากแร่ในเกม (แอตทริบิวต์ / UI เช่น `needs … strength`) เพื่อเทียบดาเมจ pickaxe; แคชสั้นๆ ต่อก้อนสำหรับ strength เท่านั้น
