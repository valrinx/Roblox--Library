...
return function(Window, runtimeInfo)
    ...
    local State = {
        ...
        AimPrediction = false,
        PredictDotSize = 6,
        PredictTargetPart = "Head", -- Head / HumanoidRootPart
    }
    local Connections = {}
    local ESPObjects = {}
    ...
    -- Calculate bullet travel time with drag
    -- Simplified drag model: v(t) = v0 * e^(-drag_coeff * t)
    -- Distance: d(t) = (v0 / k) * (1 - e^(-k*t))
    -- where k = drag factor scaled
    -- The game uses Drag as a deceleration factor per unit distance
    -- Empirical: higher Drag = slower bullet. We approximate iteratively.
    local function getBulletTravelTime(distance, muzzleVelocity, drag)
        -- Iterative approach: step through time to find when distance is reached
        local dt = 0.01
        local t = 0
        local pos = 0
        local vel = muzzleVelocity
        -- drag in this game: velocity loss per second proportional to drag value
        -- Approximate: dv/dt = -drag * constant * v
        -- Based on game data, drag ~0.7-1.2, velocity ~1400-3400
        -- Using exponential decay: v(t) = v0 * exp(-dragFactor * t)
        local dragFactor = drag * 0.3 -- empirical scaling

        for _ = 1, 2000 do -- max 20 seconds
            vel = muzzleVelocity * math.exp(-dragFactor * t)
            pos = (muzzleVelocity / dragFactor) * (1 - math.exp(-dragFactor * t))
            if pos >= distance then
                return t
            end
            t = t + dt
        end
    ...
    local function getTargetVelocity(player)
        ...
        if lastPos and lastTime then
            ...
            if dt > 0 and dt < 1 then -- only valid if recent
    ...
    local function getPredictedPosition(targetPos, targetVelocity, weaponConfig, shooterPos)
        if not weaponConfig then return targetPos end
        ...
        -- Lead: where target will be after travelTime
    ...
    local function getClosestEnemyToCrosshair()
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            ...
            local char = getCharacter(player)
            if char then
                targetPart = char:FindFirstChild(State.PredictTargetPart) or char:FindFirstChild("Head")
            end
    ...
    local function updateAimPrediction()
        ...
        if onScreen then
            ...
            -- Color based on spread (accuracy indicator)
            -- Green = low spread (accurate), Yellow = medium, Red = high
    ...
    AimTab:CreateLabel("• Dot = where to aim (compensates drop + lead)")
    AimTab:CreateLabel("• Auto-detects equipped weapon ballistics")
    AimTab:CreateLabel("• Green dot = accurate | Yellow = medium | Orange = wide spread")
